...
cron表达式 是一个字符串，通过表达式可以定义任务触发的时间

分为6或7个域，每个域的含义分别为
秒、分钟、小时、日、月、周、年
![image-20240812181428522](苍穹外卖.assets/image-20240812181428522.png)
...
基于TCP的一种新的网络协议。它实现了浏览器和服务器的全双工通信——浏览器和服务器只需要完成一次握手，两者之间就可以创建持久性的连接，并进行双向数据传输。

长连接

![image-20240812205200217](苍穹外卖.assets/image-20240812205200217.png)
